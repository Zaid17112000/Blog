<?php
    ...
    if (isset($_GET['post_id']) && is_numeric($_GET['post_id'])) {
        ...
        $queryPost = "SELECT post_title, post_content, post_excerpt, post_img_url FROM posts WHERE post_id = :post_id AND user_id = :user_id";
        ...
        if ($post) {
            ...
            $post_data['selected_categories'] = $stmt->fetchAll(PDO::FETCH_COLUMN);
            ...
            $stmt->execute(['post_id' => $post_id]);
            $post_data['selected_tags'] = $stmt->fetchAll(PDO::FETCH_COLUMN);
        } else {
    ...
    if (isset($_POST["publish"])) {
        try {
            ...
            if ($post_data['post_id']) {
                ...
                $queryUpdatePost = "UPDATE posts SET post_title = :post_title, post_content = :post_content, post_excerpt = :post_excerpt, post_img_url = :post_img_url, post_status = :post_status WHERE post_id = :post_id AND user_id = :user_id";
            ...
            if ($post_data['post_id']) {
                ...
                $previous_status = $stmt->fetchColumn();
...
<body>
        ...
        <form class="blog-form" id="blog-form" method="POST" enctype="multipart/form-data">
            ...
            <div class="button-group">
                ...
                <button type="submit" class="btn btn-primary" name="publish">
                    <?= $post_data['post_id'] ? 'Update Post' : 'Publish Now'; ?>
                </button>
            </div>
        </form>
    </div>
    ...
    <script>
        ...
        document.getElementById('save-draft').addEventListener('click', function(e) {
            ...
            fetch('../controllers/save_draft.php', {
                method: 'POST',
                body: formData
            })
            .then(response => {
